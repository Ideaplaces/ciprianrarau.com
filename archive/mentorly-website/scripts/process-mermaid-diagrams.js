...
function authorToFilename(authorName) {
  ...
  // Convert "Ciprian (Chip) Rarau" or "Ashley Werhun" to "ciprian-rarau" or "ashley-werhun"
  return authorName
    .toLowerCase()
...
function processImagesInMermaidCode(mermaidCode) {
  // Regex to find img tags with src attributes starting with /image/
  const imageRegex = /<img\s+[^>]*src=['"]\/image\/([^'"]+)['"][^>]*>/g
  ...
  while ((match = imageRegex.exec(mermaidCode)) !== null) {
    ...
    const imagePath = match[1] // e.g., "mentorly.com.png"
    const fullImagePath = path.join(PUBLIC_IMAGES_DIR, imagePath)
    
    console.log(`  🖼️  Processing image: /image/${imagePath}`)
...
// Function to create footer image with dynamic width
function createFooterImage(width = 1200) {
  console.log(`🎨 Generating footer image (width: ${width}px)...`)
  
  // Read and process the HTML template
  const htmlTemplate = fs.readFileSync(path.join(__dirname, 'create-footer.html'), 'utf8')
  const chipPhotoPath = path.join(PUBLIC_IMAGES_DIR, 'chip-small.jpg')
  
  if (!fs.existsSync(chipPhotoPath)) {
    console.log('  ❌ Chip photo not found at:', chipPhotoPath)
    return false
  }
  
  // Convert Chip's photo to base64
  const chipPhotoBase64 = convertImageToBase64(chipPhotoPath)
  if (!chipPhotoBase64) {
    console.log('  ❌ Failed to convert Chip photo to base64')
    return false
  }
  
  // Replace placeholder with actual base64 data and set width
  let htmlWithPhoto = htmlTemplate.replace('CHIP_PHOTO_BASE64', chipPhotoBase64.split(',')[1])
  htmlWithPhoto = htmlWithPhoto.replace('width: 100%;', `width: ${width}px;`)
  
  // Save the processed HTML
  const htmlOutputPath = path.join(__dirname, 'footer-temp.html')
  fs.writeFileSync(htmlOutputPath, htmlWithPhoto)
  
  console.log('  📝 Created footer HTML template')
  ...
  matches.reverse().forEach((matchData) => {
    ...
    if (imageGenerated) {
      ...
      const existingImageMatch = afterMermaidContent.match(/^\s*\n\s*!\[Diagram [^\]]*\]\([^)]+\)/)
          ...
          updatedContent = updatedContent.replace(existingImageMatch[0], `\n\n${newImageRef}`)
...
![Diagram ${diagramCount}](${cacheBustedImagePath})`
          updatedContent = updatedContent.replace(matchData.fullMatch, replacement)
